...
namespace HashTableImplmentation
{
    class MyNode
    {
    ...
    class HashTable
    {
        ...
        public HashTable(int size) //What a HashTable should have. Size and Data
        {
            this.length = size;
            this.data = new MyNodes[size];
        }

        private int hash(string key) //hashes key
        {
            int hash = 0;
        ...
        public void set(string key, int value) //Sets Key and Value to hashtable
        {
            int index = hash(key);
        ...
        public int get(string key) //Gets value associated to that KEY
        {
            int index = hash(key);
            if (this.data[index] == null)
            {
